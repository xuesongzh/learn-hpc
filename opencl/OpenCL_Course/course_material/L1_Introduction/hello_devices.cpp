...
int main(int argc, char **argv) {
    ...
    for (cl_uint n = 0; n < num_platforms; n++) {
        ...
        errcode = clGetDeviceIDs(platform_ids[n], device_type, 0, NULL, &ndevs);
        ...
        if (errcode != CL_DEVICE_NOT_FOUND) {
            ...
            // Fill device ID's
            H_ERRCHK(clGetDeviceIDs(platform_ids[n], device_type, ndevs, device_ids, NULL));
            ...
            for (cl_uint i = 0; i < ndevs; i++) {
                ...
                cl_ulong mem_size;
                H_ERRCHK(clGetDeviceInfo(device_ids[i], CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(cl_ulong),
                                         &mem_size, NULL));
